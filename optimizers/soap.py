...
def make_soap(
    ...
    def update_fn(grads, state, params):
        ...
        grad_rot = jax.tree_util.tree_map(rot, grads, q_sides)
        momentum_rot = jax.tree_util.tree_map(rot, momentum, q_sides)
        variance_rot = jax.tree_util.tree_map(lambda v, g: b2 * v + (1 - b2) * g ** 2, variance_rot, grad_rot)

        variance_rot_hat = jax.tree_util.tree_map(lambda v: v / (1 - b2 ** step), variance_rot)
        momentum_rot_hat = jax.tree_util.tree_map(lambda m: m / (1 - b1 ** step), momentum_rot)
        updates_rot = jax.tree_util.tree_map(lambda m, v: m / (jnp.sqrt(v) + eps), momentum_rot_hat, variance_rot_hat)
        ...
        variance_rot = jax.tree_util.tree_map(lambda v: jnp.where(step == 1, 0, v), variance_rot)
