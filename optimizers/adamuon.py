...
def make_adamuon(
  ...
  def init_fn(params):
    ...
    if not v_adafactor:
      ...
      v = jax.tree_util.tree_map(v_decomp, params)
  ...
  def update_fn(updates, state, params=None):
    del params
    mu = jax.tree_util.tree_map(lambda m, g: beta * m + (1 - beta) * g, state.mu, updates)
    count = state.count + 1
    mu_hat = jax.tree_util.tree_map(lambda m: m / (1 - beta ** count), mu)
    ...
    if not v_adafactor:
      v = jax.tree_util.tree_map(lambda vi, u: beta2 * vi + (1 - beta2) * (u * u), state.v, updates)
    else:
      def v_update(g, v):
          if len(g.shape) == 2:
              return [beta2 * v[i] + (1 - beta2) * jnp.mean(g ** 2, axis=1-i) for i in range(2)]
          else:
              return beta2 * v + (1 - beta2) * g ** 2
      v = jax.tree_util.tree_map(v_update, updates, state.v)
    ...
    if v_adafactor:
        def get_v_diag(v):
          if type(v) == list:
            ...
            return v
        v_form = jax.tree_util.tree_map(get_v_diag, v, is_leaf=lambda v: type(v) == list)
    else:
      v_form = v
    v_hat = jax.tree_util.tree_map(lambda vi: vi / (1 - beta2 ** count), v_form)
    updates = jax.tree_util.tree_map(
        lambda g, vi: g / (jnp.sqrt(vi) + eps).astype(vi.dtype), updates, v_hat
    )
