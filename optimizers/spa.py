...
def make_spa(
    ...
    def init_fn(params):
        ...
        def sides_decomp(p):
            if len(p.shape) == 2:
                s = [jnp.zeros((d, d)) if d < max_dim else None for d in p.shape]
                return s
            return None
        sides = jax.tree_util.tree_map(sides_decomp, params)
        def qs_decomp(p):
            if len(p.shape) == 2:
                qs = [jnp.eye(d) if d < max_dim else None for d in p.shape]
                return qs
            return None
    ...
    def update_fn(grads, state, params):
        ...
        momentum_rot = jax.tree_util.tree_map(rot, momentum, q_sides)
        signs_rot = jax.tree_util.tree_map(lambda m: jnp.sign(m), momentum_rot)
        signs = jax.tree_util.tree_map(unrot, signs_rot, q_sides)
        variance = jax.tree_util.tree_map(lambda v, g: b2 * v + (1 - b2) * g ** 2, variance, signs)
        variance_hat = jax.tree_util.tree_map(lambda v: v / (1 - b2 ** step), variance)
        updates = jax.tree_util.tree_map(lambda m, v: m / (jnp.sqrt(v) + eps), signs, variance_hat)
        sides = jax.tree_util.tree_map(update_sides, grads, sides)
        updates = jax.tree_util.tree_map(lambda u: jnp.where(step == 1, 0, u), updates)
        momentum = jax.tree_util.tree_map(lambda m: jnp.where(step == 1, 0, m), momentum)
        variance = jax.tree_util.tree_map(lambda v: jnp.where(step == 1, 0, v), variance)
    ...
    def update_slow(state, sharding, **kwargs):
        ...
        q_sides = jax.tree_util.tree_map(lambda x: jax.device_get(x), q_sides)
