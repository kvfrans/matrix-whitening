...
def make_psgd(
    ...
    def init_fn(params):
        ...
        if scanned_layers is None:
            scanned_layers_ = jax.tree_util.tree_map(lambda _: False, params)
        ...
        Qs = [
            ...
            for t, s in zip(jax.tree_util.tree_leaves(params), jax.tree_util.tree_leaves(scanned_layers_))
        ]
        ...
        Qs = [
            ...
            for q, t, s in zip(
                Qs, jax.tree_util.tree_leaves(params), jax.tree_util.tree_leaves(scanned_layers_)
            )
        ]
        Qs = jax.tree_util.tree_structure(params).unflatten(Qs)

        # Calculate sizes for nu (preconditioner) and mu (momentum)
        Qs_n_elements = sum([q.size for q in jax.tree_util.tree_leaves(Qs)])
        Qs_size_MB = sum(
            [q.size * q.dtype.itemsize / (2**20) for q in jax.tree_util.tree_leaves(Qs)]
        )
        ...
        if mu is not None:
            mu_n_elements = sum([p.size for p in jax.tree_util.tree_leaves(mu)])
            mu_size_MB = sum(
                [p.size * p.dtype.itemsize / (2**20) for p in jax.tree_util.tree_leaves(mu)]
            )
            if jax.process_index() == 0:
                print(
    ...
    def update_fn(updates: base.Updates, state: dict, params: base.Params = None):
        ...
        boxed_updates, grads_structure = jax.tree_util.tree_flatten(
        ...
        scanned_layers_ = scanned_layers
        if scanned_layers is None:
            scanned_layers_ = jax.tree_util.tree_map(lambda _: False, updates)
        ...
        if state["mu"] is not None:
            ...
            momentum_updates = jax.tree_util.tree_map(lambda m: m / (1 - b1 ** count_inc), mu)

        # flatten pytrees
        updates, grads_structure = jax.tree_util.tree_flatten(updates)
        ...
        expressions = [
            _init_Q_exprs(
                ...
                memory_save_mode,
                precond_dtype,
                existing_Q=jax.tree_util.tree_map(lambda d: d[0], Q) if s else Q,
            )
        ...
        def update_preconditioner(key, Qs):
            with jax.default_matmul_precision(precond_update_precision):
                ...
                precond_updates_in = jax.tree_util.tree_map(
        ...
        precond_gs = jax.tree_util.tree_map(_clip_fn, precond_gs)
