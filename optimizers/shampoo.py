...
def make_shampoo(
    ...
    def update_fn(grads, state, params):
        ...
        momentum_hat = jax.tree_util.tree_map(lambda m, v: m / (jnp.sqrt(v) + eps), momentum_hat, variance)
        grads = jax.tree_util.tree_map(lambda g, v: g / (jnp.sqrt(v) + eps), grads, variance)
        updates = jax.tree_util.tree_map(precondition, momentum_hat, q_sides, sides, variance_hat)            
    ...
    def update_slow(state, sharding, **kwargs):
        ...
        sides = jax.tree_util.tree_map(lambda s: s / (1 - b2 ** step), sides)
        
        q_sides = jax.tree_util.tree_map(do_inverse, sides)
